...
rails_routes: check-files
	@$(RAILS_C) routes
rspec: check-files
	@$(DC) exec -e DB_NAME=coadmap_test -e RAILS_ENV=test app bundle exec rspec --tag ~release_merge
rubocop: check-files
...
docker_build: project_env_check
	@docker build . -t gcr.io/coadmap/coadmap-${PROJECT_ENV}
docker_push: project_env_check
	@docker push gcr.io/coadmap/coadmap-${PROJECT_ENV}
